...
def test_flm_reindexing_functions(flm_generator):
    ...
    flm_1d = s2f.utils.flm_2d_to_1d(flm_2d, L)

    assert len(flm_1d.shape) == 1

    flm_2d_check = s2f.utils.flm_1d_to_2d(flm_1d, L)
...
def test_flm_reindexing_functions_healpix(flm_generator):
    ...
    flm_hp = s2f.utils.flm_2d_to_hp(flm_2d, L)

    flm_2d_check = s2f.utils.flm_hp_to_2d(flm_hp, L)
