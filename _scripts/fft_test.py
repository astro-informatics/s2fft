...
def healpix_fft(array,nside,total_pixels,shift,norm):
    ...
    for i , upper_ring in enumerate(upper_rings_offset):
        ...
        print(f"start_offset: {start_offset} end_offset: {end_offset} Size of array: {end_offset - start_offset} at {i}")
    ...
    for equator_ring in range(equator_ring_number):
        ...
        print(f"start_offset: {start_offset} end_offset: {end_offset} Size of array: {end_offset - start_offset} at {(equator_ring + nside - 1)}")
        k_a = array[start_offset:end_offset]
        #list_of_ffts.append(fshift(fft(k_a , norm=norm),shift))
    ...
    for i , lower_ring in enumerate(lower_rings_offset):
        ...
        print(f"start_offset: {start_offset} end_offset: {end_offset} Size of array: {end_offset - start_offset} at {(i + 3*nside)}")
        k_a = array[start_offset:end_offset]
        list_of_ffts.append(fshift(fft(k_a , norm=norm),shift))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s' , '--nside', type=int, help='Healpix nside' , default=4)
    parser.add_argument('-t' , '--ffttype', type=str, help = "FFTType : fwd bwd both" , default='both')
    parser.add_argument('-sh' , '--shift', action='store_true', help="fftshift result [Boolean]")
    parser.add_argument('-n' , '--norm', type=str, help='Normalization : bwd fwd ortho' , default='bwd')
    parser.add_argument('-c','--check',  action='store_true' , help='Check result [Boolean]')
    parser.add_argument('-p' , '--print' , help='Print results [Boolean]' , action='store_true')
    ...
    match fft_type:
        case FFTType.FWD:
            result = healpix_fft(a,nside,total_pixels,shift=shift_bool,norm=norm)
        case FFTType.BWD:
            result = healpix_ifft(a,nside,total_pixels,shift=shift_bool,norm=norm)
        case FFTType.BOTH:
            k_array = healpix_fft(a,nside,total_pixels,shift=shift_bool,norm=norm)
            result  = healpix_ifft(k_array,nside,total_pixels,shift=shift_bool,norm=norm)
            if args.check:
