...
def inverse(
  ...
  if spin >= 8 and method in ["numpy", "jax"]:
    raise Warning(
        "Recursive transform may provide lower precision beyond spin ~ 8")

  if method == "numpy":
    return inverse_numpy(flm, L, spin, nside, sampling, reality, precomps,
                         L_lower)
  elif method == "jax":
    return inverse_jax(flm, L, spin, nside, sampling, reality, precomps, spmd,
                       L_lower)
    ...
    return c_sph.ssht_inverse(flm, L, spin, reality, ssht_sampling,
                              _ssht_backend)
...
def inverse_numpy(
  ...
  flm[L0:] = np.einsum("lm,l->lm", flm[L0:],
                       np.sqrt((2 * np.arange(L0, L) + 1) / (4 * np.pi)))

  # Perform latitudinal wigner-d recursions
  ftm = otf.inverse_latitudinal_step(flm, thetas, L, spin, nside, sampling,
                                     reality, precomps, L0)
...
def forward_numpy(
  ...
  if sampling.lower() == "mwss":
    ...
    flm = otf.forward_latitudinal_step(ftm, thetas, L, spin, nside, sampling,
                                       reality, precomps, L0)

  # Include both pole singularities explicitly
  if sampling.lower() == "mwss":
    flm[L0:, L - 1 + spin] += (-1)**abs(np.arange(L0, L) -
                                        spin) * ftm[-1, L - 1 + spin + m_offset]
...
def forward_jax(
  ...
  r"""Compute the forward spin-spherical harmonic transform (JAX).
    ...
    Args:
        ...
        use_healpix_custom_primitive (bool, optional): Whether to use a custom CUDA primitive
                    for computing HEALPix fast fourier transform when `sampling = "healpix"` 
                    and running on a cuda compatible gpu device. 
                    using a custom primitive reduces long compilation times when jit compiling.
                    defaults to `False`.
