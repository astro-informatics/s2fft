...
def ssht_forward_precompute(f, L=4, legendre_kernel=None, device="cpu", spin=0):
    ...
    lg.debug_log("Running precompute forward ssht for L={} on {}".format(L, device))

    if device == "cpu":
        return forward_ssht_transform_cpu(f, kernel, L)
    elif device == "gpu":
        forward_jit = jit(forward_ssht_transform_gpu, static_argnums=(2,))
        return forward_jit(device_put(f), device_put(kernel), L)
    else:
        raise ValueError("Device not recognised.")


def forward_ssht_transform_cpu(f, legendre_kernel, L):
...
def forward_ssht_transform_gpu(f, legendre_kernel, L):
...
def ssht_inverse_precompute(flm, L=4, legendre_kernel=None, device="cpu", spin=0):
    ...
    lg.debug_log("Running precompute inverse ssht for L={} on {}".format(L, device))

    if device == "cpu":
        return inverse_ssht_transform_cpu(flm, kernel, L)
    elif device == "gpu":
        inverse_jit = jit(inverse_ssht_transform_gpu, static_argnums=(2,))
        return inverse_jit(device_put(flm), device_put(kernel), L)
    else:
        raise ValueError("Device not recognised.")


def inverse_ssht_transform_cpu(flm, legendre_kernel, L):
    """Computes the NumPy forward spherical harmonic transform via precompute
...
@partial(jit, static_argnums=(2,))
def inverse_ssht_transform_gpu(flm, legendre_kernel, L):
