...
if __name__ == "__main__":
    ...
    flm_shift = np.zeros_like(flm)
    flm_shift = flm

    fig, (ax1, ax2) = plt.subplots(1, 2)
    ax1.imshow(np.real(flm))
    ax2.imshow(np.real(flm_shift))
    plt.show()

    f = np.real(transform.inverse(flm, L, spin, sampling))
    f_test = np.real(inverse_transform(flm, L, spin, sampling))
    f_test_2 = np.real(inverse_transform_new(flm_shift, L, spin, sampling))
