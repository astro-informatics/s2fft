...
def inverse_wigner_transform(flmn: np.ndarray, L: int, N: int=1, sampling: str="mw") -> np.ndarray:
    ...
    """
    assert flmn.shape == wigner_samples.flmn_shape(L, N)

    if sampling not in ["mw", "mwss", "dh"]:
        raise ValueError(f"Sampling scheme sampling={sampling} not supported")

    fabn = np.zeros(wigner_samples.f_shape(L, N, sampling), dtype=np.complex128)
    
    for el in range(L):
        flmn[el,...] *= np.sqrt((2*el + 1)/(16*np.pi**3))

    for n in range(-N+1, N):
        fabn[..., N-1+n] = (-1)**n * s2f.inverse_sov_fft(flmn[..., N-1+n], L, spin=-n, sampling=sampling)
    
    return fft.ifft(fft.ifftshift(fabn, axes=2), axis=2, norm="forward")

def forward_wigner_transform(f: np.ndarray, L: int, N: int=1, sampling: str="mw") -> np.ndarray:
    ...
    """
    assert f.shape == wigner_samples.f_shape(L, N, sampling)
    ...
    for n in range(-N+1, N):
        flmn[..., N-1+n] = (-1)**n * s2f.forward_sov_fft(fabn[..., N-1+n], L, spin=-n, sampling=sampling)
