...
def mw_to_mwss_theta(f_mw: jnp.ndarray, L: int, spin: int = 0) -> jnp.ndarray:
    ...
    fmp_mwss_ext = jnp.zeros(
        ...
        dtype=_cmplx_dtype_from[str(f_mw.dtype)],
    )
    ...
    fmp_mwss_ext = fmp_mwss_ext.at[:, 1:, :].set(
        jnp.einsum(
            ...
            jnp.exp(
                -1j
                * jnp.arange(-(L - 1), L, dtype=_cmplx_dtype_from[str(f_mw.dtype)])
                * jnp.pi
                / (2 * L - 1)
            ),
            optimize=True,
...
def mw_to_mwss_phi(f_mw: jnp.ndarray, L: int) -> jnp.ndarray:
    ...
    """
    f_mwss = jnp.zeros(
        (f_mw.shape[0], L + 1, 2 * L), dtype=_cmplx_dtype_from[str(f_mw.dtype)]
    )
...
def periodic_extension(
    ...
    f_ext = jnp.zeros(
        (f.shape[0], ntheta_ext, nphi), dtype=_cmplx_dtype_from[str(f.dtype)]
    )
...
def upsample_by_two_mwss_ext(f_ext: jnp.ndarray, L: int) -> jnp.ndarray:
    ...
    f_ext_up = jnp.zeros(
        (f_ext.shape[0], ntheta_ext_up, nphi), dtype=_cmplx_dtype_from[str(f_ext.dtype)]
    )
